...
export function getMockPersonalStats(nodeId: NodeId): PersonalStats {
  ...
  return {
    ...
    directTotalInvested: String(Math.round(28000 * multiplier) * 1e6),
    totalDownstreamInvested: String(Math.round(142000 * multiplier) * 1e6),
    directCommission: String(Math.round(4200 * multiplier) * 1e18),
    teamCommission: String(Math.round(18600 * multiplier) * 1e18),
    directByTier: [
