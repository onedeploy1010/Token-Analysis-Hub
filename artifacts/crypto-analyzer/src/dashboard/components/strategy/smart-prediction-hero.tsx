 ...
 * their order flow, and recommends curated strategy packs. Node-gated.
 *
 * The "Enter" CTA opens https://www.rune-protocol.com/ in a new tab.
 * Non-node holders see a dialog that routes them to the recruit flow
 * (plain anchor to /recruit so the wouter base="/app" router doesn't
 * resolve it to /app/recruit).
 */
...
export function SmartPredictionHero() {
  ...
  return (
    <div style={{ animation: "fadeSlideIn 0.4s ease-out 0.05s both" }}>
          ...
          <div className="pt-1">
            ...
            >
              {!hasPurchased && !isLoading && (
                <Lock className="h-3.5 w-3.5 md:h-4 md:w-4 opacity-70" />
              )}
      ...
      {/* Gate: non-node user clicked Enter */}
      <Dialog open={gateOpen} onOpenChange={setGateOpen}>
        <DialogContent className="bg-card border-amber-500/30 max-w-sm">
          <DialogHeader>
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <DialogTitle className="text-base font-bold text-amber-100">
                  {t("strategy.smartPrediction.gateTitle", "Node required")}
                </DialogTitle>
                <DialogDescription className="text-[13px] text-muted-foreground mt-0.5">
                  {t(
                    "strategy.smartPrediction.gateDesc",
                    "Smart Prediction is available to RUNE node holders only. Get a node to unlock the Polymarket copy-trade feed.",
                  )}
                </DialogDescription>
              </div>
            </div>
          ...
          <DialogFooter className="gap-2 sm:gap-2 sm:flex-row flex-col">
            <Button
              variant="outline"
              onClick={() => setGateOpen(false)}
              data-testid="button-smart-prediction-gate-cancel"
            >
